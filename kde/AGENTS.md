...
- **`mise run setup:kde-fonts`** — KDE-specific font selection via `kwriteconfig6` (writes to `~/.config/kdeglobals`)
...
- **Font families**: Inter (UI), Iosevka SS08 (monospace) — installed by `setup:font-inter` and `setup:font-iosevka`.
